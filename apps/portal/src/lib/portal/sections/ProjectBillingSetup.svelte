<script lang="ts">
  ...
  function personProblems(person: (typeof people)[number]): string[] {
    const problems: string[] = [];
    if (!person.customerRate)
      problems.push(t('Set a customer hourly rate effective for this assignment.'));
    if (!person.payConfigured) problems.push(t('Set this person’s compensation method and rate.'));
    ...
    if (person.issues.includes('missing_internal_cost_rule'))
      problems.push(t('Set an internal cost rule in Finance to complete project economics.'));
    if (person.issues.some((issue) => issue.includes('ambiguous')))
      problems.push(
        t(
          'Conflicting assignments or commercial rules overlap on the effective date. Review their dates in Finance.',
        ),
      );
    if (person.issues.some((issue) => issue.includes('unavailable') || issue.includes('override')))
      problems.push(
        t(
          'A linked commercial rule is unavailable for this assignment or date. Review its scope and effective dates in Finance.',
        ),
      );
    if (person.needsReview && problems.length === 0)
      problems.push(
        t('Review the assignment and commercial rules for the effective date in Finance.'),
      );
    return problems;
  }
  ...
  async function showStep(nextStep: number): Promise<void> {
    step = nextStep;
    await tick();
    const fieldset = document.querySelector<HTMLElement>(
      '.billing-setup > form[action*="saveBillingSetup"] > fieldset',
    );
  ...
  function previous(): void {
    if (step > 1) void showStep(step - 1);
  }

  function next(): void {
    if (step < 4) void showStep(step + 1);
  }
</script>
  ...
  {#if canEdit}
    {#if setupFailure}
      <div class="warning" role="alert">
        ...
        {#if setupFailure.message}<p>{t(setupFailure.message)}</p>{/if}
        {#if setupFailure.fields}
          <ul>
            {#each Object.entries(setupFailure.fields) as [field, messages]}<li>
                {t(field)}: {localizeMessages(messages)}
              </li>{/each}
          </ul>
        {/if}
      </div>
        ...
        <fieldset>
            ...
            {#each people as person}
              ...
              {#if draft}
                <div class="person-terms">
                  ...
                  {#if personProblems(person).length > 0}
                    <ul
                      class="warning"
                      aria-label={`${person.name}: ${t('Commercial setup needs attention')}`}
                    >
                      {#each personProblems(person) as problem}<li>{problem}</li>{/each}
                    </ul>
                  {/if}
                  {#if canEdit}
          ...
          {#if missingPeople.length > 0}
            <div class="warning" role="status">
              <strong>{t('Commercial setup needs attention')}</strong>
              <p>
                {t(
              ...
              <ul>
                {#each missingPeople as person}<li>
                    <strong>{person.name}:</strong>
                    {personProblems(person).join(' ')}
                  </li>{/each}
              </ul>
            </div>
          {/if}
