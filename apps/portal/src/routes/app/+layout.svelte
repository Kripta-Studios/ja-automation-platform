<script lang="ts">
  ...
  const section = $derived($page.url.pathname.slice(`${base}/app/`.length).split('/')[0] || 'today');
  const locale = $derived((data.locale ?? 'en') as PortalLocale);
  const navigation = $derived(portalNavigationForRole(base, data.chromeUser?.role, data.chromeUser?.workforceProfile));
  ...
  async function logout() { await auth.signOut(); await goto(`${base}/app/login`); }
  function changeLocale(event: Event) { const url = new URL($page.url); url.searchParams.set('lang', (event.target as HTMLSelectElement).value); void goto(url.pathname + url.search + url.hash, { keepFocus: true, noScroll: true }); }
  ...
  beforeNavigate(({ from, to, type }) => {
    if (type !== 'popstate' && from && to && from.url.origin === to.url.origin && from.url.pathname !== to.url.pathname)
