...
## Production delivery — 9 September 2026

- Independent whole-candidate review returned **ship** after the recorded corrections. Implementation commit `edf88aa7a079fa98a59a4cc55bd7b224c94e4e30` and corrective commit `5a615d961e1dafa2d399be45ca596b139f2e6818` were pushed to `codex/v3-production-completion-orchestrated-20260819`.
- First activation migrated the database to schema41 and made the portal healthy, but the jobs preflight found a compatibility conflict: legacy missing-time jobs used the same idempotency key with a different payload. Automatic rollback attempted the schema39 application, which could not open schema41. The portal was recovered forward to the compatible image; no production database was restored or overwritten. The corrected scheduler versions the effective-calendar job key. Its legacy-coexistence/repeated-scheduling regression passed **7/7**, with an independent bounded **ship** review.
- Corrected deployment completed at **2026-09-09 00:02:29 CEST**. Active source commit: `5a615d961e1dafa2d399be45ca596b139f2e6818`. Release archive SHA-256: `4d46c495b4461cbf1bb4deebf5d0a124d4efdafe8b318674787ced3a84962697`. Active directory: `/opt/jaautomation/releases/ja-automation-4d46c495b4461cbf1bb4deebf5d0a124d4efdafe8b318674787ced3a84962697`.
- Corrective cutover backup retained at `/var/backups/jaautomation/2026-09-08T220222235Z-fa1e6161-a657-4cd6-9553-9c7349b9471a`, 29 documents, database SHA-256 `cb560f9670b03dd770ddadd7ee642cdeb7364d2bfa7e1448fe3c30c188b284b4`. Both earlier pre-migration backups are retained. Schema39 images alone are **not** a compatible rollback for the current database.
- Production verification: EN/ES/PT public pages and portal login **HTTP200** after canonical redirects; private Help and cash routes redirect anonymous requests (**303**); local readiness `ok`; SQLite full integrity `ok`, schema41, **zero foreign-key violations**. Site/portal containers became healthy, bind only loopback ports, and all five PDFs exist in the private container manual directory. `deployment/scripts/verify-vps.sh` passed; jobs run with the service actor and no scheduler conflicts.
- Delivery limitation: three approval notifications encountered **HTTP502** because their active, verified recipients have non-corporate addresses; the delivery adapter permits only `@j-aautomation.com`. Internal notices and retry state are retained. No recipient policy was relaxed, and no manual external message was sent. Successful idle job cycles do not certify delivery of those emails.
- The watcher attempted to rebuild the superseded `edf88aa` archive on its next scan. It was stopped during construction, before activation; the archive was moved outside the watched directory. The current release remained `5a615d9`. Remaining watched ZIPs were checked against processed/rejected state before watcher resumption.
- Runtime evidence: `/tmp/astra-production-verification.log`, systemd deployment journal, `/tmp/astra-rollout-job-fix.log`. Historical failed jobs/artifacts in the verifier output were not deleted or relabeled as successful.
