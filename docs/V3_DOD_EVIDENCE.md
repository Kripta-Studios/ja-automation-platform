...
| 33 immutable PDFs and traceability                             | `packages/reporting/src/exports.ts` + shared `artifact-jobs.ts`, stable artifact jobs, invoice PDF route, immutable EN/PT-BR/ES locale snapshots and the SHA-256-pinned J&A Automation logo header; `tests/reporting-artifacts.test.ts`, `tests/artifact-jobs.test.ts`, `tests/integration/invoice-lifecycle.test.ts` |
