...
| --- | --- | --- |
| Client Essential 32-step desktop journey | **30/32 step bodies passed; overall suite failed** | Steps 1–29 and 32 passed. Steps 30 and 31 could not run without fresh external operations evidence. |
...
Steps 30 and 31 require a valid operator evidence JSON through `JA_E2E_OPERATIONS_EVIDENCE` or `JA_E2E_OPERATIONS_EVIDENCE_PATH`. The contract requires recent automatic job runs and continuity backup/restore evidence or an Owner-authorized waiver. This disposable test cannot attest those external facts. The two failures were `OPERATIONS_EVIDENCE_MISSING`, not browser form failures.
