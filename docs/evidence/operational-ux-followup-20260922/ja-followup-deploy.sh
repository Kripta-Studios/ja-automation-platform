#!/usr/bin/env bash
set -Eeuo pipefail
receipt=/tmp/ja-followup-release-receipt.json
archive=$(python3 -c 'import json;print(json.load(open("/tmp/ja-followup-release-receipt.json"))["path"])')
expected=$(python3 -c 'import json;print(json.load(open("/tmp/ja-followup-release-receipt.json"))["sha256"])')
target=/home/kripta/jaautomation-operational-ux-followup-20260923.zip
[[ -f "$archive" && ! -e "$target" ]]
...
finish() {
  ...
  if [[ $result -ne 0 && -f "$target" ]]; then
    ...
    quarantine=$(mktemp -d /tmp/ja-followup-deploy-failed.XXXXXX)
