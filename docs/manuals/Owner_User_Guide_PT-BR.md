...
Use Aprovações para abrir a fila pertinente, examinar as evidências de horas/relatório/recibo e aprovar ou devolver com uma observação factual. Pesquise e filtre por trabalhador, projeto, cliente, estado e datas quando necessário. A ordem padrão mostra primeiro os registros que exigem ação, depois os de maior prioridade e, em seguida, os enviados há mais tempo. **Aprovações de projeto** reúne marcos comerciais enviados que aguardam uma decisão autorizada do projeto; aprovar um marco o libera para a próxima etapa controlada, sem aprovar todos os registros do projeto. **Revisão financeira** reúne registros operacionais cuja classificação ou confirmação financeira separada ainda está pendente. A aprovação operacional é uma revisão interna. Não é aceite do cliente, assinatura do cliente, comprovante de recebimento nem evidência de transferência.
