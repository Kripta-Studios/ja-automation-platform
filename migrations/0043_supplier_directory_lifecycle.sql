...
ALTER TABLE migration_contract_metadata RENAME TO migration_contract_metadata_v42;
CREATE TABLE migration_contract_metadata(
  migration_version INTEGER PRIMARY KEY CHECK(migration_version BETWEEN 19 AND 43),
  migration_name TEXT NOT NULL UNIQUE CHECK(migration_name IN(
    ...
    'astra_period_followup','supplier_workforce','supplier_directory_lifecycle'
  )),
...
INSERT INTO migration_contract_metadata SELECT * FROM migration_contract_metadata_v42;
...
DROP TABLE migration_contract_metadata_v42;
...
-- Reviewed directory lifecycle actions; all existing history remains immutable.
...
INSERT INTO audit_action_registry(contract_version,action,entity_type,actor_kind,owner_packet,data_classification) VALUES
  ('B5-R4','supplier.update','supplier','user','SUPPLIER-DIRECTORY','confidential'),
  ('B5-R4','supplier.status','supplier','user','SUPPLIER-DIRECTORY','confidential'),
  ('B5-R4','supplier.technician.update','user','user','SUPPLIER-DIRECTORY','confidential'),
  ('B5-R4','supplier.technician.status','user','user','SUPPLIER-DIRECTORY','confidential');
