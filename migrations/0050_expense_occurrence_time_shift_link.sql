...
ALTER TABLE migration_contract_metadata RENAME TO migration_contract_metadata_v49;
CREATE TABLE migration_contract_metadata(
  migration_version INTEGER PRIMARY KEY CHECK(migration_version BETWEEN 19 AND 50),
  migration_name TEXT NOT NULL UNIQUE CHECK(migration_name IN(
    ...
    'expense_occurrence_time_shift_link'
  )),
...
INSERT INTO migration_contract_metadata SELECT * FROM migration_contract_metadata_v49;
...
DROP TABLE migration_contract_metadata_v49;
...
-- Preserve the expense's calendar date. The optional wall-clock time is the
-- time entered at the project site, not an inferred UTC instant or shift start.
ALTER TABLE expense ADD COLUMN occurred_time_local TEXT
  CHECK(occurred_time_local IS NULL OR
    (occurred_time_local GLOB '[0-2][0-9]:[0-5][0-9]'
     AND occurred_time_local < '24:00'));
ALTER TABLE expense ADD COLUMN time_entry_id TEXT REFERENCES time_entry(id);
CREATE INDEX IF NOT EXISTS expense_time_entry_idx ON expense(time_entry_id)
