...
ALTER TABLE migration_contract_metadata RENAME TO migration_contract_metadata_v52;
CREATE TABLE migration_contract_metadata(
  migration_version INTEGER PRIMARY KEY CHECK(migration_version BETWEEN 19 AND 53),
  migration_name TEXT NOT NULL UNIQUE CHECK(migration_name IN(
    ...
    'expense_occurrence_time_shift_link','crew_leader_time','crew_expense_recorder','assignment_commercial_fallback'
  )),
...
INSERT INTO migration_contract_metadata SELECT * FROM migration_contract_metadata_v52;
...
DROP TABLE migration_contract_metadata_v52;
...
-- Global worker pay and internal-cost rules require an explicit assignment opt-in.
-- Existing rows default to off; historical issued financial snapshots are not changed.
ALTER TABLE project_member ADD COLUMN allow_global_compensation_fallback INTEGER NOT NULL DEFAULT 0 CHECK(allow_global_compensation_fallback IN (0,1));
ALTER TABLE project_member ADD COLUMN allow_global_internal_cost_fallback INTEGER NOT NULL DEFAULT 0 CHECK(allow_global_internal_cost_fallback IN (0,1));
...
INSERT INTO audit_action_registry(contract_version,action,entity_type,actor_kind,owner_packet,data_classification) VALUES
  ('B5-R4','assignment.commercial_fallback_update','project_member','user','PROJECT-COMMERCIAL','restricted'),
  ('B5-R4','assignment.commercial_rules_update','project_member','user','PROJECT-COMMERCIAL','restricted');
