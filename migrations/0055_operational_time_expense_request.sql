...
ALTER TABLE migration_contract_metadata RENAME TO migration_contract_metadata_v54;
CREATE TABLE migration_contract_metadata(
  migration_version INTEGER PRIMARY KEY CHECK(migration_version BETWEEN 19 AND 55),
  migration_name TEXT NOT NULL UNIQUE CHECK(migration_name IN(
    ...
    'expense_occurrence_time_shift_link','crew_leader_time','crew_expense_recorder','assignment_commercial_fallback','assignment_expense_policy','operational_time_expense_request'
  )),
...
INSERT INTO migration_contract_metadata SELECT * FROM migration_contract_metadata_v54;
...
DROP TABLE migration_contract_metadata_v54;
...
-- A single browser action may create one shift and one linked expense.  The
-- request binds both sources and is immutable so retries cannot duplicate costs.
CREATE TABLE operational_time_expense_request(
  ...
  time_entry_id TEXT NOT NULL UNIQUE REFERENCES time_entry(id) ON UPDATE RESTRICT ON DELETE RESTRICT,
  expense_id TEXT NOT NULL UNIQUE REFERENCES expense(id) ON UPDATE RESTRICT ON DELETE RESTRICT,
  created_at TEXT NOT NULL,
  PRIMARY KEY(actor_user_id,request_id)
) STRICT;
CREATE TRIGGER operational_time_expense_request_bind
BEFORE INSERT ON operational_time_expense_request
WHEN NOT EXISTS(
  SELECT 1 FROM time_entry t JOIN expense e ON e.id=NEW.expense_id
   WHERE t.id=NEW.time_entry_id AND e.time_entry_id=t.id
     AND e.project_id=t.project_id AND e.worker_id=t.worker_id AND e.spent_on=t.work_date
)
BEGIN SELECT RAISE(ABORT,'time expense request source mismatch'); END;
CREATE TRIGGER operational_time_expense_request_no_update
BEFORE UPDATE ON operational_time_expense_request
BEGIN SELECT RAISE(ABORT,'time expense request immutable'); END;
CREATE TRIGGER operational_time_expense_request_no_delete
BEFORE DELETE ON operational_time_expense_request
BEGIN SELECT RAISE(ABORT,'time expense request immutable'); END;
