-- Add an optional whole-project expense forecast budget without changing travel-specific budgets.
...
ALTER TABLE migration_contract_metadata RENAME TO migration_contract_metadata_v59;
CREATE TABLE migration_contract_metadata(
  migration_version INTEGER PRIMARY KEY CHECK(migration_version BETWEEN 19 AND 60),
  migration_name TEXT NOT NULL UNIQUE CHECK(migration_name IN(
    ...
    'expense_occurrence_time_shift_link','crew_leader_time','crew_expense_recorder','assignment_commercial_fallback','assignment_expense_policy','operational_time_expense_request','crew_shared_expense_allocation','combined_time_expense_billing','project_billing_setup_templates','approved_invoice_supersession','project_expense_budget'
  )),
...
INSERT INTO migration_contract_metadata SELECT * FROM migration_contract_metadata_v59;
...
DROP TABLE migration_contract_metadata_v59;
...
ALTER TABLE project ADD COLUMN expense_budget_minor INTEGER CHECK(expense_budget_minor>=0);
