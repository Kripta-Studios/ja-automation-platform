-- Permit finalizing a historical worker-paid FX settlement in receipt currency without changing invoiced source truth.
-- Register reviewed audit actions for planning assignment edits and cancellations.
...
ALTER TABLE migration_contract_metadata RENAME TO migration_contract_metadata_v63;
CREATE TABLE migration_contract_metadata(
  migration_version INTEGER PRIMARY KEY CHECK(migration_version BETWEEN 19 AND 64),
...
INSERT INTO migration_contract_metadata SELECT * FROM migration_contract_metadata_v63;
...
DROP TABLE migration_contract_metadata_v63;
...
WHEN OLD.invoice_id IS NOT NULL AND (
  ...
  (NEW.reimbursement_amount_minor IS NOT OLD.reimbursement_amount_minor AND
    NOT ((OLD.reimbursement_amount_minor IS NULL AND NEW.reimbursement_amount_minor = OLD.amount_minor) OR
      (OLD.expense_policy_required=0 AND OLD.reimbursement_state<>'reimbursed' AND
       NEW.reimbursement_state='reimbursed' AND NEW.reimbursement_amount_minor=OLD.amount_minor)))
)
BEGIN
