-- Preserve reviewed migration metadata while adding the default USD invoice issuer.
...
ALTER TABLE migration_contract_metadata RENAME TO migration_contract_metadata_v65;
CREATE TABLE migration_contract_metadata(
  migration_version INTEGER PRIMARY KEY CHECK(migration_version BETWEEN 19 AND 66),
...
INSERT INTO migration_contract_metadata SELECT * FROM migration_contract_metadata_v65;
...
DROP TABLE migration_contract_metadata_v65;
...
-- Only add the issuer when its code is absent. Existing issuer records and all
-- issued invoice snapshots remain untouched. The owner can enter a verified tax
-- identifier later; an invented tax ID would contaminate invoice evidence.
INSERT INTO legal_entity(
  id,code,legal_name,currency,billing_address,company_identifiers,
  created_at,updated_at,version,status
)
SELECT
  'legal-entity-ja-usa','JA-USA','J&A Automation LLC','USD',
  '112 Birkshire Dr, Georgetown TX 78626' || char(10) || 'Phone: +1 (864) 208 4684',
  '',datetime('now'),datetime('now'),1,'active'
WHERE NOT EXISTS(SELECT 1 FROM legal_entity WHERE code='JA-USA');
