-- Preserve reviewed migration metadata while adding the project expense reimbursement defaults.
...
ALTER TABLE migration_contract_metadata RENAME TO migration_contract_metadata_v66;
CREATE TABLE migration_contract_metadata(
  migration_version INTEGER PRIMARY KEY CHECK(migration_version BETWEEN 19 AND 67),
...
INSERT INTO migration_contract_metadata SELECT * FROM migration_contract_metadata_v66;
...
DROP TABLE migration_contract_metadata_v66;
...
-- A nullable project default preserves historical assignment policies until configured.
-- A nullable assignment override inherits the project setting. Classification snapshots
-- continue to preserve the amount selected when the expense was classified.
ALTER TABLE project ADD COLUMN worker_expense_reimbursement_default TEXT
  CHECK(worker_expense_reimbursement_default IN ('at_cost','none'));
ALTER TABLE project_member ADD COLUMN worker_expense_reimbursement_override TEXT
  CHECK(worker_expense_reimbursement_override IN ('at_cost','none'));
