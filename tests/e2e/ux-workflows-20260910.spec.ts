...
test('Owner creates a client with email and blank optional contact name', async ({ page }, info) => {
...
test('Expense defaults are valid without touching currency', async ({ page }) => {
  ...
  expect(await currency.evaluate((select: HTMLSelectElement) => new FormData(select.form!).get('currency'))).toBe('USD');
});
...
test('Registers paginate and finance configuration presents a chosen action', async ({ page }) => {
  await signIn(page, 'owner');
  for (const path of ['/projects?lang=en','/billing?lang=en','/ledger?lang=en','/accounting?lang=en']) {
    await page.goto(portal(path));
  ...
  await expect(page.getByRole('heading', { name: 'Internal loaded cost', exact: true })).toBeVisible();
});
