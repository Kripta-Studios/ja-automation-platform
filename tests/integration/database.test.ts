  ...
  it('upgrades a populated pre-V3 database without losing business rows', () => {
    ...
    try {
      ...
      sqlite
        ...
        .run(now, now);
      // Re-open through the production bootstrap so the deployment-bound
      // migration context and deterministic finance UDF are installed before
      // migrations 0019–0022 execute.
      sqlite.close();
      ...
      ).toBe(Number(files.at(-1)?.slice(0, 4)));
      expect(
        sqlite
