...
function setup() {
  ...
  return {
    ...f,
    owner: stepUpB5Principal(f.sqlite, f.owner, 'provision'),
    workforce: new SupplierWorkforceRepository(f.sqlite),
  };
}
it('provisions an external email with a verifiable hash and denies duplicate takeover', async () => {
  ...
  expect(() =>
    f.workforce.provisionLocalPortalAccount(f.worker, { ...input, email: 'other@example.test' }),
  ).toThrow();
});
it('rejects forged roles and rolls back supplier profile creation on invalid supplier', async () => {
  const f = setup();
  const passwordHash = await hashPassword('Test-only-secret-2026!');
  expect(() =>
    f.workforce.provisionLocalPortalAccount(f.owner, {
      name: 'Forged',
      email: 'forged@example.test',
      passwordHash,
      role: 'owner_admin' as 'worker',
    }),
  ).toThrow();
  expect(
    f.sqlite.prepare('SELECT id FROM user WHERE email=?').get('forged@example.test'),
  ).toBeUndefined();
  expect(() =>
    f.workforce.provisionLocalPortalAccount(f.owner, {
  ...
  expect(
    f.sqlite.prepare('SELECT id FROM user WHERE email=?').get('invalid@example.test'),
  ).toBeUndefined();
});
it('provisions read-only auditors and preserves the role boundary', async () => {
  ...
  expect(f.sqlite.prepare('SELECT role FROM user WHERE id=?').get(result.userId)?.role).toBe(
    'auditor_read_only',
  );
  expect(
    f.sqlite
      .prepare('SELECT user_id FROM supplier_user_profile WHERE user_id=?')
      .get(result.userId),
  ).toBeUndefined();
  expect(() =>
    f.workforce.setAccountProfile(f.owner, {
