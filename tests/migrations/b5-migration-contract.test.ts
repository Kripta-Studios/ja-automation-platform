...
const CONTRACT_FILES = [
  ...
  '0031_finance_source_manifest.sql',
  '0032_client_essential_worker_statement_jobs.sql',
  '0033_client_essential_service_actor_namespace.sql',
  '0034_client_essential_invoice_immutability.sql',
  ...
  '0039_mfa_optional_policy.sql',
  '0040_astra_project_closeout_revisions.sql',
  '0041_astra_period_followup.sql',
  '0042_supplier_workforce.sql',
  '0043_supplier_directory_lifecycle.sql',
  '0044_owner_record_management.sql',
  '0045_supplier_contact_directory.sql',
  '0046_worker_compensation_payments.sql',
...
const EXPECTED_MANIFEST_SHA256 = '00f70653e9f8489a3ca3dedb8e34fce4a7129c4fcb23e3262e5a09ee8331fddb';
const EXPECTED_MIGRATION_HASHES: Record<string, string> = {
  ...
  '0031_finance_source_manifest.sql':
    '07096c3408e7021234cda4ee0463f5ea6dd411fef83e741473ae3b60b46a50aa',
  '0032_client_essential_worker_statement_jobs.sql':
    '230d62c5f75f665a0e2befa123e60c497a55f0bfed5a676c98175f6d9445adb9',
  '0033_client_essential_service_actor_namespace.sql':
    '6e0780640401b4c02deb6d8aa0b8ce7315dad568d2594484baee03c529ffe2d5',
  '0034_client_essential_invoice_immutability.sql':
    'de81409516c144c9cb2f40dd7128c6f717684b24f11283ddc5c8767d4311333b',
  '0035_stalwart_mail_integration.sql':
  ...
  '0038_customer_conformity_evidence_attachment.sql':
    'dac4743859d1218d5aeb28e4276b0b7bc406f4fb19a1965e908489c89130563b',
  '0039_mfa_optional_policy.sql':
    'fa16e3dea4c7822fba1c3f41c72396c9f5d616e3614eda5da280664c42cee5c4',
  '0040_astra_project_closeout_revisions.sql':
    '34830b0a470ca87a1fa9dd6d965ce8e34520e3b25e5b8e67955443493f9d81eb',
  '0041_astra_period_followup.sql':
    '6beda397c59d516bc9079bd2778b24446fd60aa9d3ca0f113782c70fb08ce044',
  '0043_supplier_directory_lifecycle.sql':
    '120fa36be6570f9eea00fca3bb65105674a2f3bfc3477dd22913cc34bf00c3f5',
  '0042_supplier_workforce.sql': '6a6d43b7317a198ab42fb2c846dcf94f3c031d57f45a38160ca9523e212c8a9c',
  '0044_owner_record_management.sql':
    'af2c3534b8a5c46ff663434a3a6237be097b5a36ff9c050b18fbb0e16bb2744e',
  '0045_supplier_contact_directory.sql':
    'd98976eea3b20524dbfbad10ea4969a7d295f02e43ecee4f6ae1ac39f5eb2c4e',
  '0046_worker_compensation_payments.sql':
    'a60f14340d54cf9d31371852e6cf55619c240fa17065552782960ac4f566757c',
  '0047_supplier_time_batch_idempotency.sql':
    '192643d9dadeee98d6333c2d9dfc77d847ecb4c884429628f57f18b57b5c6490',
};
...
describe('frozen B5 migration contract', () => {
  ...
  it('permits the reserved synthetic Owner only for its persisted E2E deployment identity', () => {
    const insertOwner = (sqlite: DatabaseSync, id: string, email: string) => {
    ...
    const synthetic = createDatabase(':memory:').sqlite;
    try {
      expect(() =>
        insertOwner(synthetic, 'synthetic-owner', 'owner@demo.jaautomation.test'),
      ).not.toThrow();
      expect(() =>
        insertOwner(synthetic, 'canonical-owner', 'antonny.luty@j-aautomation.com'),
      ).toThrow(/designated deployment Owner/u);
    } finally {
      synthetic.close();
    }
    ...
    process.env.JA_TENANT_ID = 'test-tenant';
    process.env.JA_DEPLOYMENT_ID = 'test-deployment';
    const productionShape = createDatabase(':memory:').sqlite;
    try {
      expect(() =>
        insertOwner(productionShape, 'synthetic-owner', 'owner@demo.jaautomation.test'),
      ).toThrow(/designated deployment Owner/u);
      expect(() =>
        insertOwner(productionShape, 'canonical-owner', 'antonny.luty@j-aautomation.com'),
      ).not.toThrow();
    } finally {
      productionShape.close();
    }
  });
  ...
  it('upgrades populated supplier schema 42 and retains records, metadata and audit guards', () => {
    process.env.JA_TENANT_ID = 'test-tenant';
    process.env.JA_DEPLOYMENT_ID = 'test-deployment';
    const oldMigrations = copyMigrationTree(42);
    const allMigrations = copyMigrationTree();
    const directory = mkdtempSync(join(tmpdir(), 'ja-supplier-directory-upgrade-'));
    tempDirectories.push(directory);
    const path = join(directory, 'app.db');
    buildLegacyDatabase(path);
    process.env.JA_MIGRATIONS_PATH = oldMigrations;
    const previous = createDatabase(path);
    previous.sqlite
      .prepare(
        "INSERT INTO supplier(id,name,status,created_at,updated_at) VALUES('retained-supplier','Retained supplier','active','2026-01-01','2026-01-01')",
      )
      .run();
    const metadata = previous.sqlite
      .prepare('SELECT * FROM migration_contract_metadata ORDER BY migration_version')
      .all();
    const history = previous.sqlite.prepare('SELECT * FROM time_entry ORDER BY id').all();
    ...
    try {
      ...
      expect(
        upgraded.sqlite
          .prepare(
            'SELECT * FROM migration_contract_metadata WHERE migration_version<=42 ORDER BY migration_version',
          )
          .all(),
      ).toEqual(metadata);
      expect(upgraded.sqlite.prepare('SELECT * FROM time_entry ORDER BY id').all()).toEqual(
        history,
      );
      expect(
        upgraded.sqlite
          .prepare("SELECT name,status FROM supplier WHERE id='retained-supplier'")
          .get(),
      ).toEqual({ name: 'Retained supplier', status: 'active' });
      expect(
        upgraded.sqlite
          .prepare(
            "SELECT count(*) n FROM audit_action_registry WHERE owner_packet='SUPPLIER-DIRECTORY'",
          )
          .get(),
      ).toEqual({ n: 4 });
      expect(() =>
        upgraded.sqlite
          .prepare(
            "INSERT INTO audit_action_registry(contract_version,action,entity_type,actor_kind,owner_packet,data_classification) VALUES('B5-R4','unreviewed','user','user','SUPPLIER-DIRECTORY','confidential')",
          )
          .run(),
      ).toThrow();
  ...
  it('upgrades a populated schema-30 database without fabricating historical source manifests', () => {
    process.env.JA_TENANT_ID = 'test-tenant';
    process.env.JA_DEPLOYMENT_ID = 'test-deployment';
    const v30Migrations = copyMigrationTree(30);
    const allMigrations = copyMigrationTree();
    const directory = mkdtempSync(join(tmpdir(), 'ja-b5-v30-finance-manifest-upgrade-'));
    tempDirectories.push(directory);
    const databasePath = join(directory, 'app.db');
    buildLegacyDatabase(databasePath);

    process.env.JA_MIGRATIONS_PATH = v30Migrations;
    const v30 = createDatabase(databasePath);
    const historicalInvoices = v30.sqlite
      .prepare('SELECT id,state,snapshot_json,calculation_hash FROM invoice ORDER BY id')
      .all();
    // Stage pre-enforcement human identities as a populated production upgrade
    // would contain. Their business history remains in place; only the
    // current MFA policy projection is corrected by migration 37.
    v30.sqlite
      .prepare("UPDATE user SET mfa_required=0 WHERE id IN ('legacy-owner','legacy-worker')")
      .run();
    v30.sqlite
      .prepare(
        `INSERT INTO user(
           id,name,email,email_verified,role,status,mfa_enrolled,mfa_required,created_at,updated_at,version
         ) VALUES('legacy-invited','Legacy Invited','legacy.invited@example.test',0,'auditor_read_only','invited',0,0,?,?,1)`,
      )
      .run('2026-08-05T09:00:00.000Z', '2026-08-05T09:00:00.000Z');
    expect(v30.sqlite.prepare('SELECT MAX(version) version FROM schema_migration').get()).toEqual({
      version: 30,
    });
    v30.sqlite.close();
    ...
    try {
      ...
      expect(
        upgraded.sqlite
          .prepare('SELECT id,state,snapshot_json,calculation_hash FROM invoice ORDER BY id')
          .all(),
      ).toEqual(historicalInvoices);
      expect(
        upgraded.sqlite
          .prepare('SELECT COUNT(*) count FROM invoice_commercial_source_manifest')
          .get(),
      ).toEqual({ count: 0 });
      expect(
        upgraded.sqlite.prepare('SELECT idempotency_key FROM invoice_adjustment ORDER BY id').all(),
      ).toEqual([]);
      expect(
        upgraded.sqlite
          .prepare("SELECT id,status,mfa_required FROM user WHERE id LIKE 'legacy-%' ORDER BY id")
          .all(),
      ).toEqual([
        { id: 'legacy-invited', status: 'invited', mfa_required: 0 },
        { id: 'legacy-owner', status: 'active', mfa_required: 0 },
        { id: 'legacy-worker', status: 'active', mfa_required: 0 },
      ]);
      expect(upgraded.sqlite.prepare('PRAGMA foreign_key_check').all()).toEqual([]);
      expect(integrityCheck(upgraded.sqlite)).toBe('ok');
    } finally {
  ...
  it('upgrades a populated schema-18 copy transactionally through migration 47', () => {
    process.env.JA_TENANT_ID = 'test-tenant';
    process.env.JA_DEPLOYMENT_ID = 'test-deployment';
  ...
  it('upgrades a populated v23 copy and rolls back a failed v24 retry atomically', () => {
    ...
    try {
      ...
      expect(migrationMetadata(upgraded.sqlite)).toEqual([
        ...
        { migration_version: 31, migration_name: 'finance_source_manifest' },
        {
          migration_version: 32,
          migration_name: 'client_essential_worker_statement_jobs',
        },
        {
          migration_version: 33,
          migration_name: 'client_essential_service_actor_namespace',
        },
        {
          migration_version: 34,
          migration_name: 'client_essential_invoice_immutability',
        },
        { migration_version: 35, migration_name: 'stalwart_mail_integration' },
        { migration_version: 36, migration_name: 'time_returned_correction' },
        { migration_version: 37, migration_name: 'production_mfa_enforcement' },
        { migration_version: 38, migration_name: 'customer_conformity_evidence_attachment' },
        { migration_version: 39, migration_name: 'mfa_optional_policy' },
        { migration_version: 40, migration_name: 'astra_project_closeout_revisions' },
        { migration_version: 41, migration_name: 'astra_period_followup' },
        { migration_version: 42, migration_name: 'supplier_workforce' },
        { migration_version: 43, migration_name: 'supplier_directory_lifecycle' },
        { migration_version: 44, migration_name: 'owner_record_management' },
        { migration_version: 45, migration_name: 'supplier_contact_directory' },
        { migration_version: 46, migration_name: 'worker_compensation_payments' },
  ...
  it('upgrades a populated v22 copy with the historical metadata CHECK through 47', () => {
    process.env.JA_TENANT_ID = 'test-tenant';
    process.env.JA_DEPLOYMENT_ID = 'test-deployment';
    ...
    try {
      ...
      expect(quotedMetadataRows(upgraded.sqlite)).toHaveLength(29);
      expect(
        upgraded.sqlite
          .prepare(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name='migration_contract_metadata'",
          ...
          .get(),
      ).toMatchObject({ sql: expect.stringContaining('BETWEEN 19 AND 47') });
