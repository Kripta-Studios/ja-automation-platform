...
describe('additive Client Essential 2026-08-24 migration contract', () => {
  it('reaches the current schema and retains the reviewed migration-28 identity', () => {
    const sqlite = fresh();
    const version = (
    ...
    expect(sqlite.prepare('SELECT version FROM schema_migration ORDER BY version').all()).toEqual(
      Array.from({ length: 41 }, (_, index) => ({ version: index + 1 })),
    );
  });
  ...
  it('preserves prior reviewed migration metadata and its immutable guard', () => {
    ...
    expect(
      sqlite
        ...
        .all(),
    ).toEqual(Array.from({ length: 23 }, (_, index) => ({ migration_version: index + 19 })));
    expect(() =>
      sqlite
        .prepare(
