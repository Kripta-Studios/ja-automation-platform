...
import { browseRecords, recordDate, recordState } from '../apps/portal/src/lib/portal/ui/record-browser';
...
describe('shared record browser', () => {
  it('sorts pending reviews before approved history, oldest first within a state', () => {
    ...
    expect(browseRecords(rows, '', '', 'priority').map(row => row.id)).toEqual(['old', 'new', 'approved']);
  ...
  it('recognizes planning, reimbursement and finance DTO fields', () => {
    ...
    expect(browseRecords([{ workerName: 'Zoë' }, { workerName: 'Ana' }], '', '', 'name')[0].workerName).toBe('Ana');
    expect(browseRecords([{ client_name: 'Ácme', project_name: 'Line 4', worker_name: 'José' }], 'acme jose', '', 'priority')).toHaveLength(1);
  });
