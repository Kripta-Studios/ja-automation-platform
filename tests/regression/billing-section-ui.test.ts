...
describe('Billing section lifecycle surface', () => {
  ...
  it('renders planned versus actual dates, structured sign-off blockers and exact money through a prop', () => {
    ...
    expect(value).not.toMatch(/formatMoney\(\s*Number\s*\(/);

    // Permit only the existing non-money coercions: calendar day, bounded
    // percentage validation, and a persisted boolean. Any other use fails.
    const withoutNonMoneyCoercions = value
      .replace(/Number\(monthlyCutoffRaw\)/g, '')
      .replace(/Number\(value\) > 100/g, '')
      .replace(
        /Number\(\s*rowValue\(rule, 'auto_generate_draft', 'autoGenerateDraft'\),?\s*\)/g,
        '',
      );
    expect(withoutNonMoneyCoercions).not.toMatch(/\bNumber\s*\(/);

    const exactMoneyHelpers = value.slice(
      value.indexOf('function minorToDecimal'),
      value.indexOf('function positiveMinor'),
    );
    expect(exactMoneyHelpers).not.toMatch(/\b(?:Number|parseFloat)\s*\(/);
  });
