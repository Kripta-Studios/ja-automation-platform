...
import {
  projectCapabilityKeys,
  projectIndustryKeys,
  translateProject,
  translateServiceTags,
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const contactText = await getTranslations('contact');
  const featuredProjectCopy = featuredProjects.map((project) =>
    ...
    <>
      ...
      <section id="works" className="section-padding bg-white">
        <div className="container-ja">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {featuredProjectCopy.map((project) => (
        ...
        <div className="container-ja">
          <div className="grid gap-10 lg:grid-cols-[1fr_0.85fr] lg:items-center">
            ...
            <div className="border border-ja-line bg-ja-surface p-7 sm:p-9">
              <p className="eyebrow-steel mb-5">{remote('coverageEyebrow')}</p>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
                {[
                  'coveragePlcHmi',
                  'coverageRobotics',
                  'coverageDrivesMotion',
                  'coverageProductionStartup',
                ].map((key) => (
                  <div
                    key={key}
      ...
      <section id="contact" className="section-padding bg-ja-graphite relative overflow-hidden">
        ...
        <div className="container-ja text-center relative z-10">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6 text-sm">
            ...
            >
              {contact.primaryName} · {contactText('primaryTitle')}
            </a>
          </div>
        </div>
      </section>
